function updateSkinData(images)

% Updates the collected data of the pixels of the hands skin.
%
% Input
...
if isfile('dataset/training/skinPixels.mat')
    % skinPixels is a struct. The fields of skinPixels are the names of the
    % treated images without '.jpg' and with a f at the beginning
    data = load('dataset/training/skinPixels.mat', 'skinPixels');
    skinPixels = data.skinPixels;
else
    skinPixels = struct();
end
...
for i = 1:length(images)
    ...
    if isfield(skinPixels, strcat('f', fileName))
        alreadySeen(alreadySeenInd) = convertCharsToStrings(fileName);
        alreadySeenInd = alreadySeenInd + 1;
        continue
    end
    
    skinPixels.(strcat('f', fileName)) = cropAndLin(strcat('dataset/training/', fileName, '.jpg'));
    
    save('dataset/training/skinPixels.mat', 'skinPixels');
end
